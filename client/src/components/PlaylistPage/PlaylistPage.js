...
function PlaylistPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const playlist = await network.get(`api/v1/playlist/byId/${playlistId}`);
    ...
    <div className='playlistPage'>
      ...
      {playlist && (
        <div className='playlistPage__songBlock'>
          <SongBlock songsToRender={playlistSongs} name={playlist.name} type='playlist' typeId={playlist.playlistId} />
        </div>
      )}
    </div>
  );
