...
function AlbumPage() {
  ...
  return (
    <div className='albumPage'>
      {album && <TitleBlock type='album' id={album.id} title={album.name} description={album.description} numberOfSong={albumSongs.length} coverImg={album.coverImg} />}
      {album && (
        <div className='albumPage__songBlock'>
          <SongBlock songsToRender={album.Songs} name={album.name} type='album' typeId={album.id} />
        </div>
      )}
    </div>
  );
